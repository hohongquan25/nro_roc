...
namespace NRO_Server.DatabaseManager
{
    ...
    {
        /// <summary>
        /// Parse short[][] từ JSON — hỗ trợ cả format chuẩn [[1,2],[3,4]]
        /// lẫn double-encoded ["[1,2]","[3,4]"] do database có thể lưu cả hai.
        /// </summary>
        private static short[][] ParseShortMatrix(string json)
        {
            ...
            for (int i = 0; i < array.Count; i++)
            {
                ...
                if (token.Type == JTokenType.String)
                    result[i] = JsonConvert.DeserializeObject<short[]>(token.Value<string>());
                else
                    result[i] = token.ToObject<short[]>();
            }
            return result;
        }
                    ...
                    if (reader.HasRows)
                    {
                        while (reader.Read())
                        {
                            ...
                            if (!Cache.Gi().TASK_TEMPLATES_0.ContainsKey(id))
                                Cache.Gi().TASK_TEMPLATES_0.Add(id, taskTemplate);
                            if (!Cache.Gi().TASK_TEMPLATES_1.ContainsKey(id))
                                Cache.Gi().TASK_TEMPLATES_1.Add(id, taskTemplate);
                            if (!Cache.Gi().TASK_TEMPLATES_2.ContainsKey(id))
                                Cache.Gi().TASK_TEMPLATES_2.Add(id, taskTemplate);
                        }
                    }
                    ...
                    if (reader.HasRows)
                        while (reader.Read())
                        {
                            Cache.Gi().NRParts.Add(new Part()
                            {
                                ...
                                Data = ParseShortMatrix(reader.GetString(2))
                            });
                        }
